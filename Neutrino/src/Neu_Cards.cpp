                                   ...
                                   bool rich)
{
    ...
    if (!rich) {
        ...
        drawText(display, drawable, gc, theme, visible, x, baseline);
        return;
    }
...
void Neu_Cards::draw(Display* display, Drawable drawable, GC gc, const Neu_Theme& theme)
{
    ...
    for (size_t i = 0; i < cards_.size(); ++i) {
        ...
        int lineY = y + cardPaddingTop_ + 15;
        for (size_t itemIndex = 0; itemIndex < card.items.size(); ++itemIndex) {
            ...
            const int itemY = lineY - 14;
            int textLeft = cardX + cardPaddingLeft_;
            int textRight = cardX + cardW - cardPaddingRight_;

            if (allowIcon) {
                if (Neu_IconBmp* icon = iconForPath(item.iconPath)) {
                    const int iconY = itemY + std::max(0, (itemLineHeight_ - iconSize_) / 2);
            ...
            drawStyledTextLine(display, drawable, gc, theme, text, textLeft, lineY, textWidth, primary && item.simpleRichText);
            lineY += itemLineHeight_ + itemSpacing_;
            if (lineY > y + h - cardPaddingBottom_ + itemLineHeight_) {
                break;
            }
        }
